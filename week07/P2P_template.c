...
#include "active_address_hashmap.c"
#include "ip_address_set.c"
#include "constants.h"
...
pthread_mutex_t black_list_lock;

struct LinkedList *known_nodes;
struct Dictionary *available_files;
struct Set *black_list;
struct Hashmap *active_connections;
...
int create_introduction(char *addr, char *buffer, int len) {
    ...
    FILE *files;
    files = fopen("list_of_files", "r");

    char file[FILE_NAME_LENGTH];

    while(fgets(file, sizeof(file), files)) {
        int name_len = strlen(file);
        if (file[name_len-1] == '\n') name_len--;
        memcpy(buffer+size, file, name_len);
        size += name_len;
        buffer[size++] = ',';
    }
    buffer[--size] = '\0';
...
int get_peer_information(struct Node* node, char *buf, int len) {
    ...
    // // In order to not to send ':' without any symbol before
    // if (str_len == 0) {
    //     buf[size] = ' ';
    //     ++str_len;
    // }
...
int is_in_database(char *filename) {
    FILE *files;
    files = fopen("list_of_files", "r");
    ...
    while(fgets(file, sizeof(file), files)) {
        int name_len = strlen(file);
        if (file[name_len-1] == '\n') name_len--;
        if (strncmp(file, filename, name_len) == 0) {
            fclose(files);
            return TRUE;
        }
    }
...
void *request(void *data) {
    ...
    if (is_in_database(filename)) {
        printf("File %s is already in the database\n", filename);
        pthread_exit(NULL);
    }

    struct LinkedList *list = dict_get(filename, available_files);
    if (list == NULL) {
    ...
    /* Create a TCP socket */
    if ((sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {
        perror("socket creation while request failed");
        pthread_exit(NULL);
    }

    if (connect(sockfd, (struct sockaddr *)&dest, addr_len) < 0) {
        perror("connection while request failed");
        close(sockfd);
        pthread_exit(NULL);
    }
    ...
    printf("Sending request operation for file %s to address: %s, port: %d\n",
        filename, inet_ntoa(dest.sin_addr), ntohs(dest.sin_port));

    /* Sent request command to begin synchronization */
    if (send(sockfd, &sync_code, sizeof(int), 0) < 0) {
        perror("send request command failed");
        close(sockfd);
        pthread_exit(NULL);
    }

    /* Send name of the file */ 
    if (send(sockfd, filename, FILE_NAME_LENGTH, 0) < 0) {
        perror("send name of the file failed");
        close(sockfd);
        pthread_exit(NULL);
    }
    ...
    if ((recv_bytes = recv(sockfd, &number_of_words, sizeof(int), MSG_WAITALL)) < 0) {
        perror("recv number of words failed");
        close(sockfd);
        pthread_exit(NULL);
    } else if (recv_bytes == 0) {
        printf("stream socket peer has performed an orderly shutdown 1\n");
        close(sockfd);
        pthread_exit(NULL);
    }
    printf("Number of words: %d\n", ntohl_o(number_of_words));

    FILE *file;
    file = fopen(filename, "w");
    char word[WORD_LENGTH];

    for (int i = 0; i < ntohl_o(number_of_words)-1; ++i) {
        memset(word, 0, sizeof(word));
        if ((recv_bytes = recv(sockfd, &word, sizeof(word), MSG_WAITALL)) < 0) {
            perror("recv word failed");
            close(sockfd);
            pthread_exit(NULL);
        } else if (recv_bytes == 0) {
            printf("stream socket peer has performed an orderly shutdown 2\n");
            close(sockfd);
            pthread_exit(NULL);
        }
    ...
    if ((recv_bytes = recv(sockfd, &word, sizeof(word), MSG_WAITALL)) < 0) {
        ...
        printf("stream socket peer has performed an orderly shutdown 3\n");
        close(sockfd);
        pthread_exit(NULL);
    }
    printf("word %s accepted\n", word);
...
void *connection_handler(void *data) {
    ...
    if ((recv_bytes = recv(*sockfd, &operation, sizeof(int), MSG_WAITALL)) < 0) {
        ...
        pthread_mutex_lock(&black_list_lock);
        hashmap_delete(client_addr.sin_addr, thread, active_connections);
        pthread_mutex_unlock(&black_list_lock);
        close(*sockfd);
        free(sockfd);
        pthread_exit(NULL);
    } else if (recv_bytes == 0) {
    ...
    if (ntohl_o(operation) == 1) {
        ...
        for (int i = 0; i < ntohl_o(number_of_nodes); ++i) {
            ...
            printf("Got information about peer: %s\n", buf_peer);
            ...
            if ((node = get_known_peer(peer_addr, peer_port)) == NULL) {
                ...
                list_insert(peer_name, peer_addr, peer_port, known_nodes);
                pthread_mutex_unlock(&known_nodes_lock);
                printf("New peer %s with address %s and port %d was added\n",
                    peer_name, peer_addr, peer_port);
            } else if (strlen(node->name) == 0) {
    ...
    } else if (ntohl_o(operation) == 0) {
        char filename[FILE_NAME_LENGTH];
        memset(filename, 0, sizeof(filename));
        if ((recv_bytes = recv(*sockfd, filename, sizeof(filename), MSG_WAITALL)) < 0) {
            perror("recv filename while handling request operation failed");
            pthread_mutex_lock(&black_list_lock);
            hashmap_delete(client_addr.sin_addr, thread, active_connections);
            pthread_mutex_unlock(&black_list_lock);
            close(*sockfd);
            ...
            printf("stream socket peer has performed an orderly shutdown 8\n");
            pthread_mutex_lock(&black_list_lock);
            hashmap_delete(client_addr.sin_addr, thread, active_connections);
            pthread_mutex_unlock(&black_list_lock);
            close(*sockfd);
        ...
        printf("Request for downloading file %s\n", filename);
        ...
        counter = htonl_o(counter);

        if (send(*sockfd, &counter, sizeof(int), 0) < 0) {
            perror("send number of words while handling request operation failed");
            close(*sockfd);
            free(sockfd);
            fclose(file);
            pthread_exit(NULL);
        ...
        rewind(file);
        while (fscanf(file, "%s", word) != -1) {
            if (send(*sockfd, word, WORD_LENGTH, 0) < 0) {
                perror("send word while handling request operation failed");
                close(*sockfd);
                free(sockfd);
                fclose(file);
                pthread_exit(NULL);
...
void *clear(void* data) {
    ...
    struct ThreadList *threadList = hashmap_get_active_threads(*addr, active_connections);
    for (int i = 0; i < threadList->size; i++) {
        pthread_t threadfd = threadList->first->threadfd;
        pthread_cancel(threadfd);
        hashmap_delete(*addr, threadfd, active_connections);
    }
    pthread_mutex_unlock(&black_list_lock);
...
void *setup_tcp_server_communication(void* data) {
    ...
    server_addr.sin_port = htons(0); // Server will process any data arriving on specified port
    ...
    while(TRUE) {
        ...
        /* check whether connected ip address is in the black list */
        if (set_isIn(client_addr.sin_addr, black_list)) {
            printf("Client from the black list tried to connect: %s:%u\n", 
                inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
            close(comm_socket_fd);
            continue;
        }

        int is_added_to_bdb = FALSE;
        // pthread_mutex_lock(&active_connections_lock);
        int number_of_active_connections = hashmap_get_value(client_addr.sin_addr, active_connections);
        if (number_of_active_connections >= MAX_ACTIVE_CONNECTIONS_NUMBER) {
            set_add(client_addr.sin_addr, black_list);
            is_added_to_bdb = TRUE;
        }
        ...
        if (is_added_to_bdb) {
            printf("Client was added to the black list: %s:%u\n",
                inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
            pthread_t clear_thread;
            struct in_addr *thread_arg = malloc(sizeof(struct in_addr));
            memcpy(thread_arg, &(client_addr.sin_addr), sizeof(struct in_addr));
            pthread_create(&clear_thread, NULL, clear, (void *)thread_arg);
            continue;
        }
        ...
        *thread_arg = comm_socket_fd;
        ...
        hashmap_insert(client_addr.sin_addr, conn_thread, active_connections);
    }
...
int main() {
    ...
    if (pthread_mutex_init(&black_list_lock, NULL) != 0) {
        perror("black list mutex init failed");
        exit(EXIT_FAILURE);
    }

    // if (pthread_mutex_init(&active_connections_lock, NULL) != 0) {
    //     perror("active nodes mutex init failed");
    //     exit(EXIT_FAILURE);
    // }

    known_nodes = list_create();
    available_files = dict_create();
    black_list = set_create();
    active_connections = hashmap_create();
    ...
    while(TRUE) {
        nread = getline(&file, &len, stdin);
        if (nread == -1) {
            perror("line was not read");
            continue;
        }
        ...
        if (strcmp(file, "exit\n") == 0) {
            printf("Exit command. Process terminates.\n");
            return 0;
        }

        if (strncmp(file, "recv ", 5) == 0) {
            char filename[FILE_NAME_LENGTH];
            memset(filename, 0, sizeof(filename));
            memcpy(filename, file+5, strlen(file)-6);
            
            pthread_t request_thread;
            pthread_create(&request_thread, NULL, request, (void*)filename);
        } else {
            printf("Unknown command. Type 'recv name_of_file' to receive a file\n");
        }
    };
}